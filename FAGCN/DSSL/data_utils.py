...
def sample_neg_neighborhood(dataset, device, args):
    neighbor_max = args.neighbor_max
    adjacent_list = {}
    for i in range(0, dataset.graph['num_nodes']):
        adjacent_list[i] = dataset.graph['edge_index'][1, dataset.graph['edge_index'][0] == i]

    for i in range(0, dataset.graph['num_nodes']):
        perm = torch.randperm(dataset.graph['num_nodes'])#.to(device)
...
def sample_neighborhood(dataset, device, args):
    neighbor_max = args.neighbor_max
    adjacent_list = {}
    for i in range(0, dataset.graph['num_nodes']):
        adjacent_list[i] = dataset.graph['edge_index'][1, dataset.graph['edge_index'][0] == i]
    ...
    for i in range(0, dataset.graph['num_nodes']):
        ...
        if num_neighbor < neighbor_max:
            more = math.ceil(neighbor_max / num_neighbor) if num_neighbor != 0 else 0
            if num_neighbor == 0:
                adjacent_list[i]=torch.zeros(5).cuda()
            else:
...
def rand_balance_idx(label, c, train_prop=.01, valid_prop=.05, ignore_negative=True):
    if ignore_negative:
        labeled_nodes = torch.where(label != -1)[0]
    else:
        labeled_nodes = label
    ...
    # get num for each class
    train_class_num = math.ceil(train_num / c)
    valid_class_num = math.ceil(valid_num / c)
    # get idx for each class
    train_indices = []
    valid_indices = []
    test_indices = []
    for ci in range(c):
        class_idx = torch.where(labeled_nodes == ci)[0]
        perm = torch.as_tensor(np.random.permutation(class_idx.shape[0]))
        train_class_idx = class_idx[perm[:train_class_num]]
        valid_class_idx = class_idx[perm[train_class_num:train_class_num + valid_class_num]]
        test_class_idx = class_idx[perm[train_class_num + valid_class_num:]]
        train_indices.append(train_class_idx)
        valid_indices.append(valid_class_idx)
        test_indices.append(test_class_idx)

    train_indices = torch.hstack(train_indices)
    val_indices = torch.hstack(valid_indices)
    test_indices = torch.hstack(test_indices)
...
@torch.no_grad()
def evaluate(out, data, split_idx, eval_func):
    train_acc = eval_func(
...
def load_fixed_splits(dataset, sub_dataset):
    ...
    if sub_dataset and sub_dataset != 'None':
        name += f'-{sub_dataset}'
    print (f'./data/splits/{name}-splits.npy')
