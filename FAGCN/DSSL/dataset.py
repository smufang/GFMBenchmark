...
import csv
import json
from DSSL.data_utils import rand_train_test_idx, even_quantile_labels, to_sparse_tensor, dataset_drive_url
...
def load_tudataset(name):
    data = torch.load(f"/home/xingtong/DSSL/data/{name}/data.pkl")
    edge_index = data.edge_index
    node_feat = data.features
    label = data.labels
...
# def load_geom_gcn_dataset_Wikipedia(name):
#     torch_dataset = WikipediaNetwork(root=f'{DATAPATH}/WikipediaNetwork',
#                               name=name, geom_gcn_preprocess=True, transform=T.NormalizeFeatures())
#     data = torch_dataset[0]
#     num_nodes = data.num_nodes
#     print(f"Num nodes: {num_nodes}")
#     return data
# def load_geom_gcn_dataset_WebKB(name):
#     torch_dataset = WebKB(root=f'{DATAPATH}/WikipediaNetwork',
