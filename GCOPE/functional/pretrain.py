...
@param('general.save_dir', 'save_dir')
...
def run(
    ...
    if (saliency_model == 'mlp'):
        # load data
        from GCOPE.data import get_clustered_data
        data = get_clustered_data(dataset, model_id=model_id)
        ...
        with torch.no_grad():
            data, gco_model, raw_data = get_clustered_data(dataset, model_id=model_id)
    ...
    # save
    import os
    pt_path = os.path.join(save_dir, ','.join(dataset) + '_pretrained_model.pt')
    torch.save(model.state_dict(), pt_path)
    print('Pretrained model saved to:', pt_path)
...
def graph_cl_pretrain(
    ...
    def get_loaders(data, batch_size):
        ...
        from GCOPE.algorithm.graph_augment import graph_views
    ...
    if (gco_model == None):
        if (reconstruct == 0.0):
            ...
            optimizer = torch.optim.Adam(
                filter(lambda p: p.requires_grad,
                       list(model.parameters()) + list(loss_fn.parameters()) + list(rec_loss_fn.parameters())),
                lr=learning_rate,
                weight_decay=weight_decay
            )
    else:
        if (reconstruct == 0.0):
            optimizer = torch.optim.Adam(
                filter(lambda p: p.requires_grad,
                       list(gco_model.parameters()) + list(model.parameters()) + list(loss_fn.parameters())),
                lr=learning_rate,
                weight_decay=weight_decay
            )
        else:
            ...
            optimizer = torch.optim.Adam(
                filter(lambda p: p.requires_grad,
                       list(gco_model.parameters()) + list(model.parameters()) + list(loss_fn.parameters()) + list(
                           rec_loss_fn.parameters())),
                lr=learning_rate,
                weight_decay=weight_decay
            )
    ...
    from GCOPE.data.contrastive import update_graph_list_param
    ...
    for e in range(epoch):
        ...
        pbar = tqdm(zip(*loaders), total=len(loaders[0]), ncols=100, desc=f'Epoch {e}')
...
def simgrace_pretrain(
    ...
    from torch_geometric.loader import DataLoader
    from GCOPE.data import gen_ran_output
