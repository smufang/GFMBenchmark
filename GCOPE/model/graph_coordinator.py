...
class GraphCoordinator(nn.Module):
    ...
    def add_learnable_features_with_no_grad(self, original_node_features):
        # adds learnable parameters as additional features to the original node features.
    ...
    def forward(self, batch_with_no_grad_node_features):
        # replaces the learnable_features_with_no_grad (adding in the func before) with grad
        count = 0
        graph_index_list = [x for x in set(batch_with_no_grad_node_features.batch.tolist())]
        for graph_index in graph_index_list:
            node_indices_corresponding_graph = (batch_with_no_grad_node_features.batch == graph_index).nonzero(
...
if __name__ == '__main__':
    ...
    print(model.learnable_param)
    graph_coordinator = torch.cat([p.data.unsqueeze(0) for p in model.learnable_param], dim=0)
    print(graph_coordinator)