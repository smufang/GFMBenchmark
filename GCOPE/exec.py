...
Section('general', 'General Configs').params(
    ...
    model_id=Param(str, default='exp1', desc='model id for saving/loading models'),
    exp_id=Param(str, default='exp1', desc='experiment id'),
    ssl_weight=Param(float, default=1.0),
...
Section('data', 'Data Configs').params(
    name=Param(SubsetOf([
        ...
        'cora', 'citeseer', 'pubmed', 'computers', 'photo', 'pretrain', 'node', 'edge', 'graph', 'test'
    ]), required=True),
    seed=Param(int, default=777, desc='seed for train/val/test split, fix this to get the same dataset'),
    node_feature_dim=Param(int, default=0,
...
Section('pretrain', 'Pretraining Configs').enable_if(
    lambda cfg: cfg['general.func'] in ['pretrain']
).params(
    ...
    dynamic_edge=Param(OneOf(['internal', 'external', 'internal_external', 'similarity', 'none']), default='none'),
...
def run(func, seed, name):
    ...
    import_module(f'GCOPE.functional.{func}').run()
