...
def get_clustered_data(dataset, cache_dir, cross_link, cl_init_method='learnable', cross_link_ablation=False,
                       dynamic_edge='none', dynamic_prune=0.0, split_method='RandomWalk', model_id='exp1'):
    from GCOPE.data.utils_gcope import preprocess, iterate_datasets
    ...
    if 'pretrain' in dataset:
        ...
        data_list = [preprocess(data) for data in dataset_lst]
    elif 'test' in dataset:
        from data_provider.data_loader import get_original_data
        from data_provider import TEST_pt
        dataset_dict = get_original_data(TEST_pt)
        dataset_lst = dataset_dict.values()
        print('Loading dataset...')
        data_list = [preprocess(data) for data in dataset_lst]
    else:
        # preprocess: align the node features by svd and padding
    ...
    if (cross_link > 0):
        ...
        if (cl_init_method == 'mean'):
            # Create coordinator nodes for each batch, using the mean features of nodes within that batch - domain
            new_node_features = []
            ...
            # Create coordinator nodes for each batch, using the sum features of nodes within that batch - domain
            new_node_features = []
            for node_graph_index in new_index_list:
                node_indices_corresponding_graph = (data.batch == node_graph_index).nonzero(as_tuple=False).view(-1)
            ...
            # Create coordinator nodes for each batch, using a simple constant feature vector: [1,...,1]
            new_node_features = torch.ones((len(new_index_list), data.num_node_features))
            data.x = torch.cat([data.x, new_node_features], dim=0)
        elif (cl_init_method == 'learnable'):
            # Create coordinator nodes for each batch, using learnable parameters
            from GCOPE.model.graph_coordinator import GraphCoordinator
        ...
        # add batch indices for new nodes
        ...
        if (dynamic_edge == 'none'):
            if (cross_link == 1):
                for node_graph_index in new_index_list:
                    ...
                    new_edges = torch.cartesian_prod(torch.tensor([new_node_index]),
                                                     node_indices_corresponding_graph[:-1])
                    data.edge_index = torch.cat([data.edge_index, new_edges.t()], dim=1)

                    new_edges = torch.cartesian_prod(node_indices_corresponding_graph[:-1],
                ...
                for node_graph_index in new_index_list[:num_graphs]:
                    ...
                    for new_node_index in new_node_index_list:
                        ...
                        new_edges = torch.cartesian_prod(node_indices_corresponding_graph[:-1 * cross_link],
                                                         torch.tensor([new_node_index]))
                        data.edge_index = torch.cat([data.edge_index, new_edges.t()], dim=1)
            ...
            for node_graph_index in new_index_list:
                node_indices_corresponding_graph_for_simi = (
                        data_for_similarity_computation.batch == node_graph_index).nonzero(as_tuple=False).view(-1)
                graph_mean_features.append(
                    torch.mean(data_for_similarity_computation.x[node_indices_corresponding_graph_for_simi],
                               dim=0).tolist())
            ...
            if ((data.edge_index.shape[1] - total_edge_num_before_add_cross_link_internal_edges) ==
...
if __name__ == '__main__':
    ...
    data_dict = get_original_data(pretrain, 100)
    ...
    Section('general').params(cache_dir=Param(Folder(True), default='storage/.cache'))
    Section('data', 'Data Configs').params(
        node_feature_dim=Param(int, default=100))
    Section('pretrain').params(
        ...
        cl_init_method=Param(OneOf(['mean', 'sum', 'learnable', 'simple', 'none']),
                             default='learnable'),
        split_method=Param(OneOf(['metis', 'RandomWalk']), default='RandomWalk'),
    )

    graph_list, gco_model, raw_data = get_clustered_data(dataset)
