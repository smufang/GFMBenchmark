...
class ExpDownstreamBatch(ExpBasic):
    def __init__(self, args, pretrain_dict, name, dataset):
        ...
        if self.compress_func is not None:
            ...
            try:
                ...
                self._print_main("Not PyG Data() with x attribute.")

        if self.args.model in ['heco']:
            self.args.feature_dim = list(self.data.num_node_features.values())
            self.args.metapaths = auto_generate_metapaths(self.data)

        if self.args.model != "none":
            if self.args.model_id == 'none':
                self._print_main(f"{self.args.model} model, no pretraining loaded.")
                self.pretrain_model =  None
            else:
                self.pretrain_model = self._get_pretrain_model()
    ...
    def _build_model(self):
        model = self.model_dict[self.args.model].Downstream(self.args, self.pretrain_model, self.data.clone().cpu()).float()
    ...
    def _get_pretrain_model(self):
        ...
        if self.args.pattern == 'single':
            pretrain_model = (
                self.model_dict[self.args.model].Pretrain(self.args, self.data).float()
            )
        else:
            pretrain_model = (
    ...
    def run_model_cache(self, task, is_train, cache=None):
        ...
        if self.args.preprocess == 'basic':
            ...
            data = self.data
        # for Heterogeneous Methods
        if self.args.model in ['simple_hgn']:
            data.node_type = self.data.node_type
            data.edge_type = self.data.edge_type
    ...
    def run_model(self, task, is_train):
        ...
        if is_train is True:
            avg_loss = total_loss / task_labels.size(0)
            return avg_loss
        else:
            labels = torch.cat(labels, dim=0)
            preds = torch.cat(preds, dim=0)
    ...
    def run_tasks(self):
        ...
        for task_dim, fewshot_tasks in fewshot_tasks_dict.items():
            ...
            for idx, train_task in enumerate(pbar, start=1):
                ...
                self.optimizer, scheduler = self._select_optimizer()
                self.model.train()
