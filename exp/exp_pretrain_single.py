...
class ExpPretrainSingle(ExpBasic):
    def __init__(self, args, pretrain_dict):
        super(ExpPretrainSingle, self).__init__(args, pretrain_dict)
        self._print_main(f'Pretrain datasets: {", ".join(self.pretrain_dict.keys())}')
        assert len(self.pretrain_dict) == 1, "Single-pattern pretrain expects exactly one dataset."
        self.dataset = self.pretrain_dict[list(self.pretrain_dict.keys())[0]]()
        preprocess_map = {
    ...
    def _build_model(self):
        model = self.model_dict[self.args.model].Pretrain(self.args, self.data).float()
    ...
    def _get_loader(self):
        ...
        elif self.args.preprocess == "simple":
            name, data = next(iter(self.pretrain_dict.items()))
            self._print_main(f"Using single pretrain dataset: {name}")
            data = self._simple_preprocess(data())
            if self.data_type == "temporal":
                from torch_geometric.loader import TemporalDataLoader
                return TemporalDataLoader(data, batch_size=self.args.batch_size, neg_sampling_ratio=self.args.num_neg_samples)
            else:
                return [data]
        else:
            raise ValueError(f"Unknown preprocess mode{self.args.preprocess}.")
            ...
            for batch_idx, batch_data in enumerate(pbar, start=1):
                ...
                if self.args.use_amp:
                    with torch.amp.autocast("cuda"):
                        if self.args.model in ['ddgcl']:
                            loss, k, t_past = self.model(batch_data)
                        else:
                ...
                if self.args.model in ['tgn']:
                    self.model.update(batch_data)
                    num_batch_tasks = batch_data.src.size(0)
                if self.args.model in ['ddgcl']:
                    self.model._dequeue_and_enqueue(k, t_past)
