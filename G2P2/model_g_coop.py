...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        x = x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection # [batch_size, transformer.width] @ [transformer.width, embed_dim] -> [batch_size, embed_dim]
...
class CustomCLIP(nn.Module):
    def __init__(self, args, classnames, clip_model, g_texts):
        ...
        self.edge_linear = nn.Linear(args.embed_dim, 2*args.embed_dim)
    ...
    def forward(self, data, idx):
        image_features = self.image_encoder(data.x, data.edge_index)
        if self.args.task_name == 'node':
            image_features = image_features[idx] # [selected_nodes, hidden_dim]
        elif self.args.task_name == 'edge':
            select_src_idx, select_dst_idx = idx
            # Using subtraction for edge representation: concat would yield [B, 2*hidden_dim],
            # which mismatches text feature dim. Subtraction keeps [B, hidden_dim] and encodes direction (src - dst).
            #image_features = (image_features[select_src_idx] + image_features[select_dst_idx]) /2 # [selected_edges, hidden_dim]
            image_features = torch.cat([image_features[select_src_idx], image_features[select_dst_idx]], dim=-1)
        elif self.args.task_name == 'graph':
            graph_embed = torch_scatter.scatter(image_features, data.batch, dim=0, reduce='mean') 
            image_features = graph_embed[idx] # [selected_graphs, hidden_dim]

        prompts = self.prompt_learner()
        text_features = self.text_encoder(prompts, self.tokenized_prompts)

        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True) # [n_cls, hidden_dim]
        if self.args.task_name == 'edge':
            text_features = self.edge_linear(text_features) # [n_cls, 2*hidden_dim]

        logit_scale = self.logit_scale.exp()
        logits = logit_scale * image_features @ text_features.t() # [selected_items, n_cls]
...
class CoOp(nn.Module):
    ...
    def __init__(self, args, classnames, clip_model, g_texts, device):
        ...
        for name, param in self.model.named_parameters():
            if "prompt_learner" not in name and "edge_linear" not in name:
