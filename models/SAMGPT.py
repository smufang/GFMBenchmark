...
class Pretrain(nn.Module):
    def __init__(self, configs):
        ...
        self.structure_prompt_layers = nn.ModuleList(
            [AlignPrompt(hidden_dim, domain_id=self.domain_id, combinetype=combinetype) for _ in range(num_layers)])
    ...
    def compute_linkpred_prelogits(self, data):
        ...
        str_prelogits = self.lp(self.gnn_enc, data, prompt_layers=self.structure_prompt_layers)
        return fea_prelogits + self.alpha * str_prelogits
    ...
    def compute_graphcl_prelogits(self, data, aug_type='edge',
        ...
        str_prelogits, str_lbl = self.graphcl(self.gnn_enc, data, samp_bias1, samp_bias2, aug_type=aug_type,
                                              drop_percent=self.drop_percent,
                                              prompt_layers=self.structure_prompt_layers)

        if not torch.equal(fea_lbl, str_lbl):
            raise ValueError("Feature and Structure branches produced different labels.")

        return fea_prelogits + self.alpha * str_prelogits, fea_lbl
    ...
    def get_prompt_weights(self):
        return {
            ...
            "structure_prompt": [layer.prompt.detach() for layer in self.structure_prompt_layers],
            "combine_weight": self.alpha
        }
...
class Downstream(nn.Module):
    def __init__(self, configs, pretrain_model, dataset):
        ...
        assert 'feature_prompt' in prompt_weights, "feature_prompt must be provided for SAMGPT downstream tasks."
        assert 'structure_prompt' in prompt_weights, "structure_prompt must be provided for SAMGPT downstream tasks."
        feature_prompt_weights = prompt_weights['feature_prompt']
        structure_prompt_weights = prompt_weights['structure_prompt']
        ...
        self.str_holistic_prompts = nn.ModuleList(
            [AlignPrompt(hidden_dim, domain_id=downstream_domain_id, combinetype=combinetype) for _ in range(num_layers)])
        self.str_specific_prompts = nn.ModuleList(
            [ComposedPrompt(structure_prompt_weights[i], combinetype=combinetype) for i in range(num_layers)]
        )
    ...
    def embedding_forward(self, data):
        ...
        fea_al_embed = fea_hol_embed + self.beta * fea_spe_embed
        fea_al_embed = self.gnn_enc(fea_al_embed, data.edge_index, prompt_layers=None, name=data.name)

        str_hol_embed = self.gnn_enc(data.x, data.edge_index, prompt_layers=self.str_holistic_prompts, name=data.name)
        str_spe_embed = self.gnn_enc(data.x, data.edge_index, prompt_layers=self.str_specific_prompts, name=data.name)
        str_al_embed = str_hol_embed + self.beta * str_spe_embed

        return fea_al_embed + self.alpha * str_al_embed
