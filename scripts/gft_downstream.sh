#!/bin/bash
#nohup bash ./scripts/gft_downstream.sh > gft_downstream_run.log 2>&1 &
...
echo "To stop gft_downstream.sh: kill $$"
echo "======================================"

PID_FILE=logs/gft/gft_down_${EXP}_${TASK_NAME}_${SHOT}_${timestamp}.pid

for SHOT in "${NUM_SHOTS[@]}"
do
    for dataset in "${DATASETS[@]}"
    do
        LOGFILE=logs/gft/gft_down_${EXP}_${TASK_NAME}_${dataset}_${SHOT}_${timestamp}.log
        
        echo "[START] dataset=${dataset}, shot=${SHOT}"
        echo "Log file: ${LOGFILE}"

        nohup python -u GFT/GFT/finetune.py \
                --exp ${EXP} \
                --task_name ${TASK_NAME} \
                --n_shot ${SHOT} \
                --gpu ${GPU} \
